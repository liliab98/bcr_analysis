...
# Boxplots
plot_clonotype_boxplot <- function(data_long) {
  ggplot(data_long, aes(x = Chain, y = Unique_Clonotypes, fill = Chain)) +
    geom_boxplot(outlier.size = 0.5) +
    theme_minimal() +
    labs(title = NULL, x = NULL, y = "Unique Clonotypes") +
...
bladder_unique_clontype_plot <- plot_clonotype_boxplot(bladder_clonotype_long)
renal_unique_clontype_plot <- plot_clonotype_boxplot(renal_clonotype_long)
...
# Plot boxplot
plot_read_depth_boxplot <- function(data_long) {
...
bladder_read_depth_plot <- plot_read_depth_boxplot(read_depth_bladder)
renal_read_depth_plot   <- plot_read_depth_boxplot(read_depth_renal)
...
# Bar Proportion plot
plot_expansion_proportion <- function(expanded_data) {
  ggplot(expanded_data, aes(x = "", fill = expanded)) +
    ...
    labs(x = NULL, y = NULL, fill = "Expansion Status") +
    facet_wrap(~ Chain) +
    theme_minimal() +
    theme(
      legend.position = "none",
...
bladder_plot <- plot_expansion_proportion(bladder_expanded_clonotypes_all)
renal_plot <- plot_expansion_proportion(renal_expanded_clonotypes_all)

print(bladder_plot)
print(renal_plot)

# Save the plot as .pdf
#ggsave("bladder_expansion_proportion.pdf", plot = bladder_expanded_proportion_chain, width = 6, height = 5, units = "in")
#ggsave("renal_expansion_proportion.pdf", plot = renal_expanded_proportion_chain, width = 6, height = 5, units = "in")
...
plot_expansion_frequency <- function(summary_df, fill_colors = c("Expanded" = "orangered", "Non-Expanded" = "royalblue")) {
...
bladder_expanded_freq_chain <- plot_expansion_frequency(bladder_abundance_summary)
renal_expanded_freq_chain   <- plot_expansion_frequency(renal_abundance_summary)
